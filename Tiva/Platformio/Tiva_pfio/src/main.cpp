...
uint8_t serialBuffer[BUFFERSIZE] = {0};
double vImag[BUFFERSIZE] = {0};

int serialPeaks[BUFFERSIZE] = {0};
int serialPeaks2[BUFFERSIZE] = {0};

float signal_filtered = 0;
uint16_t filtered = 0;
...
void loop()
{
  ...
  if (measure)
  {
    ...
    if (trigger)
    {
      serialBuffer[send] = (uint8_t)filtered;
      ...
      if (send == BUFFERSIZE)
      {
        ...
        for (i = 0; i < BUFFERSIZE; i++)
        {
          ...
          lastPeak = serialPeaks[i];
          max_val = max(max_val, serialBuffer[i]);
          min_val = min(min_val, serialBuffer[i]);
        }

        vpp = max_val - min_val;
        //  min_val = min_val * (3.3 / 254.0);
        //  max_val = max_val * (3.3 / 254.0);
